...
public class BookGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all books").get("/api/books").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new book")
                        ...
                        .body(
                            StringBody("{" + "\"title\": \"SAMPLE_TEXT\"" + ", \"isbn\": \"SAMPLE_TEXT\"" + ", \"avaliable\": null" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created book").get("${new_book_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created book").delete("${new_book_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
