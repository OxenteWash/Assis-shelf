 ...
 */
public class LoanGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all loans").get("/api/loans").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new loan")
                        .post("/api/loans")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"loanTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"returnTimeExpected\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"realReturnTime\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_loan_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created loan").get("${new_loan_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created loan").delete("${new_loan_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Loan entity").exec(scn);
