  ...
  return (
    <div>
      <h2 id="book-heading" data-cy="BookHeading">
        <Translate contentKey="washLibraryApp.book.home.title">Books</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="washLibraryApp.book.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/book/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="washLibraryApp.book.home.createLabel">Create new Book</Translate>
          </Link>
        </div>
      </h2>
        ...
        {bookList && bookList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="washLibraryApp.book.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('avaliable')}>
                  <Translate contentKey="washLibraryApp.book.avaliable">Avaliable</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('avaliable')} />
                </th>
                <th>
                  <Translate contentKey="washLibraryApp.book.category">Category</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {bookList.map((book, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{book.title}</td>
                  <td>{book.isbn}</td>
                  <td>{book.avaliable ? 'true' : 'false'}</td>
                  <td>
                    {book.categories
                      ? book.categories.map((val, j) => (
                          <span key={j}>
                            <Link to={`/category/${val.id}`}>{val.id}</Link>
                            {j === book.categories.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="washLibraryApp.book.home.notFound">No Books found</Translate>
            </div>
          )
        )}
      </div>
